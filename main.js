...
ipcMain.on('form_data',function(e,form_data){  
  ...
  ticket_password = form_data['ticket_pwd'];
  console.log('ticket_password:'+ticket_password);
  ...
  console.log('client_id new : '+client_id);console.log('asset_id new : '+asset_id);console.log('user_id new : '+user_id);
 console.log('asset_id'+asset_id);
  var body = JSON.stringify({ "funcType": 'ticketInsert', "tic_type": form_data['type'], "loginID": loginid, "calender": calendar_id,
    ...
    "accept": is_accept, "wi_step": resolver_wi_step, "partner_tic": is_partner_ticket, "ticket_password": ticket_password }); 
  ...
  request.on('response', (response) => {
      ...
      response.on('data', (chunk) => {
        ...
        if (chunk) {
          ...
          try {
            var obj = JSON.parse(chunk);
            console.log("TicketSql: "+obj.sql12);
            logEverywhere("TicketInsert: "+obj.status);
            logEverywhere("Ticketsql: "+obj.sql);
            var result = [];
            if(obj.status == 'valid'){
              global.ticketNo = obj.ticket_no;
              result['status'] = 1;
              result['ticketNo'] = ticketNo;
              result['ticket_status'] = obj.ticket_status;
...
ipcMain.on('loadAllocUser',function(e,data){ 
  require('dns').resolve('www.google.com', function(err) {
   console.log('Inside loadAllocUser Call');
  // logEverywhere('In loadAllocUser');
   if (err) {
       console.log("No connection");
    } else {
    //  logEverywhere('In loadAllocUser');
      ...
      //  logEverywhere('In loadAllocUser');
          var body = JSON.stringify({ "funcType": 'getAllocRoom', "user_id": data.user_id }); 
          const request = net.request({ 
              method: 'POST', 
              url: root_url+'/login.php' 
          }); 
                ...
                if (chunk) {
                  ...
                  try {
                    ...
                    if(obj.status == 'valid'){
                      ...
                      return console.log('getAllocRoom: No proper response received'); // error in the above string (in this case, yes)!
                  }
                 } 
              })
              response.on('end', () => {})
          ...
          request.on('error', (error) => { 
              log.info('Error while getting allocated room detail '+`${(error)}`)
          })
          request.setHeader('Content-Type', 'application/json'); 
          request.write(body, 'utf-8'); 
          request.end();
...
ipcMain.on('login_data',function(e,data){ 
  ...
  hdd_total = 0;
   ...
   console.log(sys_OEM);console.log(sys_model);console.log(value);console.log(data.system_key); console.log(getmac.default());
    ...
    console.log('UPs No: ', data.ups_no);
    console.log('Monitor No: ', data.monitor_no);
    console.log('Room No:', data.roomId);
    console.log('Department No:', data.departmentId);  
    console.log('CPU No:', data.usr_first_name);  
    console.log('User Id:', userId);  
    var body = JSON.stringify({ "funcType": 'loginFunc', "userID": userId,
      ...
      "user_email": data.usr_email,"user_mob_no": data.usr_contact,"token": data.token,"client_no": data.clientno,"ip": system_ip,"make":sys_OEM, "model": sys_model, "serial_num": value, "mac_address": mac_address, "deviceId": deviceId, "monitor_no": data.monitor_no, "ups_no": data.ups_no, "room_id": data.roomId, "department_id": data.departmentId}); 
    const request = net.request({ 
        method: 'POST', 
        url: root_url+'/login.php' 
    }); 
...
ipcMain.on('check_member_email',function(e,form_data){ 
  console.log("checkmemberemail : "+form_data['email']);
...
autoUpdater.on('update-downloaded', () => {
  notifier.notify(
    {
      title: 'ITAM Version 1.0.1 Released. Click to Restart Application.', //put version number of future release. not current.
                ...
                logEverywhere(obj);
                  if(obj.status == 'valid'){
                    ...
                    if (process.platform === 'win32') {
                      ...
                      child_process.exec(`"C:\\Users\\%USERNAME%\\AppData\\Local\\Programs\\BrainwareUniversity\\Uninstall BrainwareUniversity.exe" --uninstall`, (err, stdout, stderr) => {
                        console.log(err);
                        console.log(stdout);
                        console.log(stderr);
...
function softwareDetails()
{
  require('dns').resolve('www.google.com', function(err) {
    ...
    logEverywhere('Inside Check Software Call in log');
     if (err) {
        console.log("No connection");
     } else {
       session.defaultSession.cookies.get({ url: 'http://www.eprompto.com' })
